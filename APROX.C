...
#include <dos.h>
#include <setjmp.h>
static const M='Í';/* MASK */
#define _(_P_)  (_P_^M),
static char user[]=
#include "user.hpp" /* wymuszenie kompilacji modulu dla nowego uzytkownika */
jmp_buf post_error;

#define ABORT 0
void SetInterrups();
#pragma startup SetInterrups 255
void RestoreInterrups();
#pragma exit RestoreInterrups 255


int cdecl c_break(void)
{
   printf("Control-Break pressed - procedure termination ...\n");
	PAUSE;
...
void cdecl Catcher(int sig/*, int type, int *reglist*/)
{
#ifdef DEBUG
   fprintf(stderr,"\nSorry ! Floating point error (%d).",sig);
#else
   fprintf(stderr,"\nSorry ! Floating point error ");
#endif
   PAUSE;
   _clear87();
   _fpreset();
   signal(SIGFPE, Catcher); /* Bo jest usuwany ! */
   longjmp(post_error,2);                                          
   exit(-4);
}


#define INTDIV0  0  /* Divide by 0 iterrupt */

void interrupt ( *oldhandler)(...);
void interrupt ( *old0x1)(...);
void interrupt ( *old0x3)(...);

void interrupt handler(...)
{
fprintf(stderr,"\nSorry ! Divide by 0 "PENTER);
getch();
longjmp(post_error,3);
}

void interrupt abort_handler(...)
{/* By zycie hackera bylo jeszcze ciezsze */
do{
int i=random(10);
switch(i){
case 0:asm int 18H;
...
ctrlbrk(c_break);
/* instal Floating error handler */
#ifndef DEMO
signal(SIGFPE, Catcher);
#endif
/* save the old interrupt vector */
oldhandler = getvect(INTDIV0);
old0x1 = getvect(0x1);
old0x3 = getvect(0x3);
#ifndef NHandleDIV0
/* install the new interrupt handler */
setvect(INTDIV0 , handler);
#endif
#ifdef ANTY_DEBUG
do{/* Przechwycenie przerwan typowego debuggera */
   setvect(0x1 , abort_handler);
   setvect(0x3 , abort_handler);
   i++;
   }while(i<105);
#endif
}
...
void StandardGenApprox(band* STD,int N)
{
...
for(ster=0;ster<N;ster++)
	{
	STD[ster].kbz=log10(STD[ster].kbz);
	}
for(ster=0;ster<N;ster++)
	{
...
for(ster=0;ster<N;ster++)
	{
	STD[ster].kbz=exp(ln10* STD[ster].kbz);
	}
}

float StandardApproxymation(float X)
{
...
for(i=0;i<N;i++) STD[i].kbz=STD[i].kbz*STD[i].kbz;
}
...
GramSmitGenApprox(STD , N);
for(i=0;i<N;i++) STD[i].kbz=exp( STD[i].kbz );
}
...
GramSmitGenApprox( STD , N);
for(i=0;i<N;i++) STD[i].kbz=exp( ln10*STD[i].kbz );
}
...
Approxymation= HiperApproxymation;break;
  case StandType:
genApproxymation= StandardGenApprox;
Approxymation= StandardApproxymation;break;
  case PolyType:

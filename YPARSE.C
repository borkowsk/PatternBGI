...
{ /* LABEL */
			...
			if( lanenum>=MAX_lane_NUMBER)
				{
				yyerror("T O O   M A N Y   L A N E S  !!!");
				return 1;
				}

			actual_lane=lanes+lanenum;
			actual_lane->label=yypvt[-0].str;
			actual_lane->not_empty=1;

			actual_lane->all_kill=0;
			actual_lane->fresh=0;
			actual_lane->done=0;
			actual_lane->stand=0;
...
{ /* Label : lista_prazkow ; */
			...
			actual_lane->band_number=bandnum;
			actual_lane->fresh=1;
			pom=(band*)realloc(actual_lane->bands,sizeof(band)*(1+bandnum));
			if(pom==NULL)
				{
...
{/* l i s t a _ p r a z k o w --- CONST*/
			actual_lane->bands[0].met=yypvt[-0].flo;
			actual_lane->bands[0].kbz=0;
			actual_lane->bands[0].eps=epsilon;
			bandnum=1;
			} break;
case 20:
# line 152 "patt2dat.y"
{/* lista , -//- */
			...
			if(lanenum==0)
				{
				yyerror("Illegal operator -//- in first lane");
				return 1;
				}
			for(i=bandnum;i<lanes[lanenum-1].band_number;i++)
				{
				actual_lane->bands[i]=lanes[lanenum-1].bands[i];
				bandnum++;
				}
			} break;
case 21:
...
{/* lista , CONST */
			...
			if(bandnum>=MAX_BANDS_NUMBER)
				{
				yyerror("Too many bands");
				return 1;
				}
			actual_lane->bands[bandnum].met=yypvt[-0].flo;
			actual_lane->bands[bandnum].kbz=0;
			actual_lane->bands[bandnum].eps=epsilon;

			if(
			actual_lane->bands[bandnum].met<actual_lane->bands[bandnum-1].met
				)
				warning("Incorrect sequen of values ");
			bandnum++;
			} break;
case 23:
# line 206 "patt2dat.y"
{/* l i s ta _ s t a n d a r t u --- CONST = CONST */
			...
			actual_lane->bands[0].met=yypvt[-2].flo;
			actual_lane->bands[0].kbz=yypvt[-0].flo;
			actual_lane->bands[0].eps=0;
...
{/* lista , CONST = CONST */
			...
			actual_lane->bands[bandnum].met=yypvt[-2].flo;
			actual_lane->bands[bandnum].kbz=yypvt[-0].flo;
			actual_lane->bands[bandnum].eps=0;

			if(
			actual_lane->bands[bandnum].met<actual_lane->bands[bandnum-1].met
				)
				warning("Incorrect sequen of standard values ");
			if(
			actual_lane->bands[bandnum].kbz>actual_lane->bands[bandnum-1].kbz
				)


				warning("Incorrect sequen of standard data values ");
			bandnum++;
			} break;
case 25:
# line 239 "patt2dat.y"
{/* l i s t a _ d o n e  --- CONST = CONST +- CONST */
			...
			actual_lane->bands[0].met=yypvt[-5].flo;
			actual_lane->bands[0].kbz=yypvt[-3].flo;
			actual_lane->bands[0].eps=yypvt[-0].flo;
			bandnum=1;
			} break;
case 26:
# line 248 "patt2dat.y"
{/* lista , CONST = CONST +- CONST */
			...
			actual_lane->bands[bandnum].met=yypvt[-5].flo;
			actual_lane->bands[bandnum].kbz=yypvt[-3].flo;
			actual_lane->bands[bandnum].eps=yypvt[-0].flo;

			if(
			actual_lane->bands[bandnum].met<actual_lane->bands[bandnum-1].met
				)
				warning("Incorrect sequen of done values ");
			if(
			actual_lane->bands[bandnum].kbz>actual_lane->bands[bandnum-1].kbz
				)
				warning("Incorrect sequen of done data values ");
			bandnum++;
			} break;
case 27:
# line 271 "patt2dat.y"
